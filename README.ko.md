...
**GPU 단계**. 원본에서 조각을 잘라 하나의 규격으로 재인코딩하고, 이어붙여 S3 에 올린다.

[English README](README.md)

## 개요

원본 영상과 그룹별 구간을 받아, 각 구간을 자르고 그룹마다 타이틀 카드(**범퍼**)를 앞에 붙인
뒤 순서대로 이어 `{v_id}/{v_id}_{c_id}.mp4` 를 만든다.
...
```
요청 {그룹: [구간들]}
[1] check_video    — 원본 존재·영상 여부·구간 유효성            (HTTP 계층, 404/400)
[2] to_media_parts — 그룹 → 평평한 조각 목록, 그룹마다 범퍼 삽입
[3] cut_parts      — 조각마다 ffmpeg 한 번, 원본 규격으로 재인코딩  (GPU, 순차)
[4] render         — concat demuxer + stream copy → work/final.mp4 → 결과물로 이동
[5] s3 업로드      — {S3URL}/{v_id}/{v_id}_{c_id}.mp4 + 구간 목록 .json
```

1~2 는 요청 스레드에서, 3~5 는 **워커 스레드 하나**에서 돈다.
...
순차로 돌면 조각이 몇 개든 VRAM 이 500 MB 대로 일정하다. FastAPI 는 `def` 핸들러를 자기
40개짜리 스레드풀에서 돌리므로, 여기로 모으지 않으면 동시 요청 수만큼 ffmpeg 이 뜬다.

**`-c copy` 가 아니라 재인코딩하는 이유.** stream copy 는 키프레임에서만 시작할 수 있다. 이
원본은 키프레임이 5초 간격인데 하이라이트 평균 길이가 16초라, copy 로 자르면 모든 컷이 최대
5초씩 어긋난다. 게다가 ffmpeg 은 그 초과분을 mp4 *edit list* 로 감추는데 concat demuxer 는
그걸 무시한다 — 조각 하나씩 보면 멀쩡한데 이어붙이면 앞부분이 되살아난다
...
lib/
  ...
  http/
    ...
    http_util.py             요청/응답 로깅, 에러 규격, check_video()
  svc/
...
INPUT_DATA_DIR="data/input"          # 원본, 그리고 종목별 하위 폴더의 범퍼
OUTPUT_DATA_DIR="data/output"        # 결과물과 작업 디렉토리

S3URL="s3://<bucket>/result"         # 비우면 로컬에만 남긴다
```
...
| `BUMPER_DATA_DIR` | `{baseball: …/baseball}` | 종목별 타이틀 카드 위치 |
...
| `file_name` | 원본 파일명, **`INPUT_DATA_DIR` 기준 상대 경로** |
...
`start_sec` / `end_sec` 만 사용한다. `start_hms` / `end_hms` 는 디버깅 표기용으로 그대로
실려 나가며 신뢰하지 않는다.
...
`concat` 은 무시해도 된다: 200 MB 이어붙이기에 0.26초로, 자르기의 60분의 1이다.
